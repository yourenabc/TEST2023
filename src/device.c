...
int rs485_1_Init(void)
{
    ...
    ctx_rs485_1 = modbus_new_rtu("/dev/ttyUSB0", 9600, 'N', 8, 1);
    //设置从机地址
...
int send_modbus_cmd (int funcode, int addr, int nb, uint16_t *dest)
{
    ...
    if(funcode == 1)
    {
        result = modbus_read_bits(ctx_rs485_1, addr, nb, dest);
    }
    else if(funcode == 2)
    {
        result = modbus_read_input_bits(ctx_rs485_1, addr, nb, dest);
    }
    else if(funcode == 3)
    {
        result = modbus_read_registers(ctx_rs485_1, addr, nb, dest);
    }
    else if(funcode == 4)
    {
        result = modbus_read_input_registers(ctx_rs485_1, addr, nb, dest);
    }
    else
    {
        result = modbus_read_registers(ctx_rs485_1, addr, nb, dest);
                ...
                if(module_config_buf[ii].slave_addr == module_config_buf[ii+1].slave_addr)
                {
                    ...
                    else
                    {
                        if(consecutive_sum == 0)//如果跟下一个不连续且第一次进来 直接输出
                        {
                            ...
                            rc = send_modbus_cmd(module_config_buf[ii].storage_type, module_config_buf[ii].data_addr, single_num, &data[data_offset]);
                        ...
                        else
                        {
                            ...
                            consecutive_sum = consecutive_sum + single_num;
                            modbus_set_slave(ctx_rs485_1, module_config_buf[ii].slave_addr);
                            rc = send_modbus_cmd(module_config_buf[ii].storage_type, consecutive_addr_begin, consecutive_sum, &data[data_offset]);
...
void timerTestInit(void)
{
    ...
    new_timer.it_value.tv_sec = 20;//10sh后开始执行
